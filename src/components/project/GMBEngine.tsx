...
const GMB_CATEGORIES = [
  'GMB Post', 'GMB Q&A', 'GMB Image Upload', 'GMB Review Reply', 'GMB Offer'
];

export const GMBEngine = ({ onOpenAction, onOpenKeywords }: { onOpenAction: (category: string) => void, onOpenKeywords: () => void }) => {
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
       <div className="flex justify-between items-center mb-8 border-b border-slate-200/60 pb-6">
         <div>
           ...
           <p className="text-slate-500">GMB Management Dashboard</p>
         </div>
         <button 
              ...
              <div key={cat} className="bg-white border border-slate-100/50 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_40px_rgb(0,0,0,0.08)] rounded-3xl p-6 transition-all duration-300 group hover:-translate-y-1 relative">
                  ...
                  <div className="absolute top-6 right-6">
                    <span className="bg-blue-50 text-blue-600 text-xs font-black tracking-wider px-3 py-1 rounded-full border border-blue-100">
                      STEP {String(GMB_CATEGORIES.indexOf(cat) + 1).padStart(2, '0')}
                    </span>
                  </div>
                  ...
                  <div className="mb-8">
                    <div className="flex justify-between text-sm mb-2 font-medium">
                      ...
                      <span className="text-emerald-600">{completed} / {total}</span>
                    </div>
                  ...
                  <div className="flex flex-col 2xl:flex-row gap-3">
                    <button 
                      ...
                      className="flex-1 bg-gradient-to-r from-emerald-500/10 to-teal-500/10 hover:from-emerald-500 hover:to-teal-500 hover:text-white text-emerald-700 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2 group/btn border border-emerald-500/10 hover:border-transparent hover:shadow-lg hover:shadow-emerald-500/20"
                    >
                      <Plus className="w-4 h-4 group-hover/btn:scale-110 transition-transform" /> Submit
                    </button>
                    <button 
